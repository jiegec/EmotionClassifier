...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(128, 128, 5).to(device)
        self.conv2 = nn.Conv1d(128, 128, 5).to(device)
        self.max1 = nn.MaxPool1d(5).to(device)
        self.max2 = nn.MaxPool1d(5).to(device)
        self.linear = nn.Linear(128 * 162, num_emotions).to(device)
...
def print_accuracy():
    ...
    print('train correct/all: %d/%d' % (correct, correct + wrong))
